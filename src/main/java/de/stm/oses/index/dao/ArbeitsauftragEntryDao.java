...
public interface ArbeitsauftragEntryDao {
    ...
    @Transaction
    @Query("SELECT * FROM arbeitsauftrag WHERE bezeichner = :bezeichner AND (datum = :datum OR (datum_von <= :datum AND datum_bis >= :datum)) ORDER by datum_von, last_edit ASC LIMIT 1")
    ArbeitsauftragWithFileEntry getFileForVerwendung(String bezeichner, Date datum);
